...
/**
 * Open tags: HTML allows `/` between the tag name and attributes
 * (`<img/src=...>`); only `/>` is self-closing.
 */
...
/** Whitespace the URL parser strips before scheme parsing (not U+0020). */
const URL_STRIP_WHITESPACE_RE = /[\t\n\f\r]+/g;

/**
 * Decode entities, strip ASCII whitespace, and normalize `\` → `/` so
 * htt&tab;ps://, semicolonless numeric refs, and \\evil.com/… cannot evade.
 */
function normalizeForRemoteUrlCheck(value) {
  return decodeHtmlCharacterReferences(value)
    .replace(ASCII_WHITESPACE_RE, "")
    .replace(/\\/g, "/");
}
...
function* srcsetUrlTokens(input) {
  ...
  while (i < input.length) {
    ...
    if (input.slice(i, i + 5).toLowerCase() === "data:") {
      // data:[header],[payload...] — commas belong to the URL until space.
      ...
      // Non-data: URL continues through commas until a space separator.
      let end = i;
      while (end < input.length && input[end] !== " ") end += 1;
      url = input.slice(i, end);
      i = end;
...
function srcsetHasRemoteUrl(value) {
  // Decode entities; strip URL-ignored whitespace but keep SPACE so candidate
  // boundaries stay intact (commas inside a URL token are not separators).
  const forParse = decodeHtmlCharacterReferences(value).replace(URL_STRIP_WHITESPACE_RE, "");
  for (const url of srcsetUrlTokens(forParse)) {
    const normalized = url.replace(ASCII_WHITESPACE_RE, "").replace(/\\/g, "/");
