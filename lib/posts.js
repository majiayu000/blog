 ...
 * `/>` is self-closing.
 *
 * Namespace tracking uses a stack (`svg` / `html`) so a nested `<svg>` inside
 * `foreignObject` re-enters SVG context (where `use` / `feimage` fetch).
 */
function* iterateOpenTags(html) {
  ...
  /** @type {Array<"svg" | "html">} */
  const nsStack = [];
  const inSvgNs = () => nsStack.length > 0 && nsStack[nsStack.length - 1] === "svg";
  /** Pop through integration frames until (and including) the matching kind. */
  const popNsUntil = (kind) => {
    for (let n = nsStack.length - 1; n >= 0; n -= 1) {
      if (nsStack[n] === kind) {
        nsStack.length = n;
        return;
      }
  ...
  while (i < text.length) {
    ...
    if (next === "!" && inSvgNs() && text.startsWith("<![CDATA[", start)) {
    ...
    // HTML raw-text/RCDATA elements ignore the self-closing flag (see below).
    const selfClosing = /\/[\t\n\f\r ]*$/.test(attrRegion);
    const attrText = attrRegion.replace(/\/[\t\n\f\r ]*$/, "");
    const inSvgContext = inSvgNs();
    yield { tag, attrText, inSvgContext };

    // Foreign elements honor `/>`; do not push namespace for self-closing svg /
    // foreignObject. (HTML raw-text ignores `/>` for body scanning only.)
    if (!selfClosing) {
      if (tag === "svg") nsStack.push("svg");
      else if (tag === "foreignobject" && inSvgContext) nsStack.push("html");
    }
    ...
    // HTML ignores the self-closing flag on raw-text elements (`<script/>`
    // still expects `</script>`); only foreign-content self-closing skips the body.
    if (RAW_TEXT_TAGS.has(tag) && !(tag === "title" && inSvgContext)) {
      if (!(selfClosing && inSvgContext)) {
