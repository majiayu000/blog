...
const REMOTE_RESOURCE_ATTRS = new Set(["href", "src", "data", "poster"]);
...
/** Absolute http(s) or protocol-relative // URL (after trimming). */
...
function hasRemoteResourceTag(html) {
  ...
  for (const match of html.matchAll(OPEN_TAG_RE)) {
    ...
    for (const attr of REMOTE_RESOURCE_ATTRS) {
      if (attr in attrs && isRemoteResourceUrl(attrs[attr])) return true;
    }
  }
  return false;
}
