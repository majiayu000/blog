...
function cssTextHasRemoteResource(cssText, options = {}) {
  ...
  const withHtmlRefs = decodeHtml
    ...
    : String(cssText);
  const decoded = decodeCssEscapes(withHtmlRefs);
  if (REMOTE_CSS_RESOURCE_RE.test(decoded)) return true;
  // Also catch `url(https:evil.com/…)` forms that omit `//` after decode.
  const urlRe = /(?:@import\s+(?:url\(\s*)?|url\(\s*)["']?([^"')\s]+)/gi;
  let match;
...
function scanRemoteCssResource(html, options) {
  for (const { tag, attrText, styleBody } of iterateOpenTags(html, options)) {
    const attrs = parseHtmlAttrs(attrText);
...
function* iterateOpenTags(html, options = {}) {
  ...
  while (i < text.length) {
    ...
    // In "in select" / "in frameset" insertion modes, most start tags are
    // ignored — including RAWTEXT openers such as `<style>`. Entering RAWTEXT
    // here would skip past a following remote `<frame>` / `<img>` the browser
    // loads. `<script>` is still processed (in-head rules); keep RAWTEXT for it.
    const ignoredInSelectOrFrameset =
      (inSelectMode() || inFramesetMode()) &&
      parentNs === "html" &&
      tag !== "script" &&
      (RAW_TEXT_TAGS.has(tag) || (tag === "noscript" && noscriptRawtext));
    if (ignoredInSelectOrFrameset) {
      yield openTag;
      i = j + 1;
      continue;
    }
    ...
    // RAWTEXT, but its text content is still an active stylesheet — capture
    // the body through the validated `</style>` closer for remote-CSS checks.
    const isHtmlRawTextElement =
      (RAW_TEXT_TAGS.has(tag) || (tag === "noscript" && noscriptRawtext)) &&
      parentNs === "html";
    const isSvgStyleElement = tag === "style" && parentNs === "svg";

    if ((isHtmlRawTextElement || isSvgStyleElement) && !honorSelfClosing) {
