...
function decodeHtmlCharacterReferences(value) {
  return String(value).replace(
    /&(?:#x([0-9a-fA-F]+)(?:;|(?![0-9a-fA-F]))|#([0-9]+)(?:;|(?![0-9]))|([A-Za-z][A-Za-z0-9]*);)/g,
 ...
 * integration points (`foreignObject` / `desc` / `title`) and ignores foreign
 * end tags the HTML tree builder would drop (e.g. `</svg>` while a special
 * HTML `div` is current).
 *
 * @param {string} html
  ...
  while (i < text.length) {
    ...
    while (j < text.length) {
      ...
      if (isAsciiWhitespace(ch)) {
        inAttrName = false;
        j += 1;
        // Whitespace after attributes clears a pending self-closing solidus
        // only when we are still looking for `/` before `>`; keep scanning.
        continue;
      }
      if (ch === "/") {
