 ...
 * first `>` ends the declaration.
 */
function skipBogusCommentOrDeclaration(text, start) {
  // `<!DOCTYPE` (ASCII case-insensitive) — quote-aware closer.
  if (
    text[start + 1] === "!" &&
    ...
    while (i < text.length) {
      ...
      if (ch === "[") {
        // Bogus DOCTYPE — first `>` ends the declaration (no subset rules).
...
/** True when CSS text (after HTML + CSS decoding) loads a remote url/@import. */
function cssTextHasRemoteResource(cssText) {
  const decoded = decodeCssEscapes(decodeHtmlCharacterReferences(cssText));
  ...
  while ((match = urlRe.exec(decoded)) !== null) {
    if (isRemoteResourceUrlDecoded(normalizeForRemoteUrlCheck(match[1]))) return true;
  }
  return false;
}
...
/**
 * Scan style attributes and HTML-namespace `<style>` bodies for remote CSS.
 * Relies on `iterateOpenTags` open-element tracking so RAWTEXT/`select`/
 * `frameset` rules match the resource-tag scanner.
 */
function scanRemoteCssResource(html, options) {
  for (const { tag, attrText, styleBody } of iterateOpenTags(html, options)) {
    ...
    if (tag === "style" && styleBody !== undefined && cssTextHasRemoteResource(styleBody)) {
      return true;
    }
  }
  return false;
  ...
  while (i < text.length) {
    ...
    if (isHtmlRawTextElement && !honorSelfClosing) {
      ...
      if (tag === "plaintext") {
        ...
        if (tag === "style") {
          // Body ends at the matched end tag (skip* advances past it).
          let bodyEnd = i;
          const closeAt = indexOfAsciiIgnoreCase(text, "</style", bodyStart);
          if (closeAt !== -1 && closeAt < i) bodyEnd = closeAt;
          openTag.styleBody = text.slice(bodyStart, bodyEnd);
        }
 ...
 * `1e0x`) then `x`. A trailing decimal point without a following digit
  ...
  for (const { tag, attrText, inSvgContext } of iterateOpenTags(html, options)) {
    ...
    if (tag === "input") {
      const type = asciiLowerCase(
        decodeHtmlCharacterReferences(attrs.type ?? "text").trim(),
      );
