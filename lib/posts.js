...
/** Absolute http(s) or protocol-relative // URL (after trimming + entity decode). */
function isRemoteResourceUrl(value) {
  return isRemoteResourceUrlDecoded(decodeHtmlCharacterReferences(value).trim());
}
...
function srcsetHasRemoteUrl(value) {
  for (const url of srcsetUrlTokens(decodeHtmlCharacterReferences(value))) {
    if (isRemoteResourceUrlDecoded(url.trim())) return true;
  }
  return false;
}
