 ...
 *
 * A bare `[` after the DOCTYPE name is a parse error that enters the bogus
 * DOCTYPE state (ends at the first `>`). Only after a completed public/system
 * identifier may `[`…`]` open an internal subset where `>` is ordinary text.
 */
function skipBogusCommentOrDeclaration(text, start) {
  ...
  if (
    ...
    let quote = null;
    let inSubset = false;
    // True only after a quoted public/system identifier finishes — HTML allows
    // `[` internal subset there. A bare `[` after the name is bogus DOCTYPE.
    let allowInternalSubset = false;
    while (i < text.length) {
      const ch = text[i];
      if (inSubset) {
      ...
      if (ch === "[") {
        if (allowInternalSubset) {
          inSubset = true;
          i += 1;
          continue;
        }
        // Bogus DOCTYPE — first `>` ends the declaration (no quote/subset rules).
...
function hasRemoteCssResource(html) {
  if (REMOTE_CSS_RESOURCE_RE.test(html)) return true;
  if (cssTextHasRemoteResource(html)) return true;
  for (const { attrText } of iterateOpenTags(html)) {
    const attrs = parseHtmlAttrs(attrText);
...
function* iterateOpenTags(html, options = {}) {
  ...
  while (i < text.length) {
    ...
    // In "in select" insertion mode, most start tags are ignored — including
    // RAWTEXT openers such as `<style>`. Entering RAWTEXT here would skip past
    // `</select>` and hide a following remote `<img>` the browser loads.
    // `<script>` is still processed (in-head rules); keep RAWTEXT for it.
    const ignoredInSelect =
      inSelectMode() &&
      parentNs === "html" &&
      tag !== "script" &&
