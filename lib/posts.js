...
function* iterateOpenTags(html) {
  ...
  while (i < text.length) {
    ...
    const attrText = text.slice(attrStart, j).replace(/\/\s*$/, "");
    yield { tag, attrText };
    i = j + 1;
  }
}
...
/**
 * C0 controls + SPACE (U+0000–U+0020). URL parsing strips these at the edges;
 * removing them throughout also catches scheme-embedded tabs and leading VT.
 */
const URL_C0_OR_SPACE_RE = /[\u0000-\u0020]+/g;

/**
 * Decode entities, strip C0/space, and normalize `\` → `/` so htt&tab;ps://,
 * leading &#11;, semicolonless numeric refs, and \\evil.com/… cannot evade.
 */
function normalizeForRemoteUrlCheck(value) {
  return decodeHtmlCharacterReferences(value)
    .replace(URL_C0_OR_SPACE_RE, "")
    .replace(/\\/g, "/");
}

/**
 * Probe base for special-scheme resolution. Off-origin http(s) hosts are remote;
 * `http:evil.com/x` resolves to `http://evil.com/x` even without `//`.
 */
...
function isRemoteResourceUrlDecoded(trimmed) {
  ...
  try {
    const resolved = new URL(trimmed, REMOTE_URL_PROBE_BASE);
    if (resolved.protocol !== "http:" && resolved.protocol !== "https:") return false;
    return Boolean(resolved.host) && resolved.host !== "blog.invalid";
  } catch {
...
function isRemoteResourceUrl(value) {
  ...
  // Fail closed: undecoded named refs in resource URLs are not trusted local paths.
  return /&[a-z][a-z0-9]*;/i.test(String(value));
}

/**
 * HTML srcset density/width descriptors. Density uses a valid non-negative
 * floating-point number (including scientific notation like `1e0x`) then `x`.
 */
const SRCSET_DESCRIPTOR_RE =
  /^(?:(?:\d+\.\d+|\d+\.?|\.\d+)(?:[eE][+-]?\d+)?|\d+[eE][+-]?\d+)x$|^\d+w$/;
...
function srcsetHasRemoteUrl(value) {
  ...
  for (const forParse of passes) {
    for (const url of srcsetUrlTokens(forParse)) {
      const normalized = url.replace(URL_C0_OR_SPACE_RE, "").replace(/\\/g, "/");
...
function hasRemoteResourceTag(html) {
  for (const { tag, attrText } of iterateOpenTags(html)) {
