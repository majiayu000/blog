...
function skipRawTextBody(text, from, tag) {
  ...
  while (pos < text.length) {
    ...
    if (ch === ">" || ch === "/" || isAsciiWhitespace(ch)) {
      ...
      return k < text.length ? k + 1 : text.length;
    }
    pos = closeAt + 1;
  }
  return text.length;
...
function* iterateOpenTags(html) {
  ...
  while (i < text.length) {
    ...
    const attrText = text.slice(attrStart, j).replace(/\/[\t\n\f\r ]*$/, "");
    yield { tag, attrText };
    i = j + 1;
...
function* srcsetUrlTokens(input) {
  ...
  /** Advance to the next top-level comma; commas inside `(…)` stay in-descriptor. */
  ...
  const skipDescriptors = () => {
    ...
    while (i < input.length && input[i] !== ",") {
      const start = i;
      let depth = 0;
      // Srcset descriptor tokenizer parenthesis state: only whitespace / comma
      // at depth 0 ends a descriptor token.
      while (i < input.length) {
        const ch = input[i];
        if (ch === "(") {
          depth += 1;
        ...
        if (depth === 0 && (isAsciiWhitespace(ch) || ch === ",")) break;
        i += 1;
      }
