...
/** Strip CSS block comments, replacing each with a single space. */
...
/** True when CSS text (after optional HTML + CSS decoding) loads a remote url/@import. */
function cssTextHasRemoteResource(cssText, options = {}) {
  // HTML character references are decoded in style *attributes*, but not inside
  // an HTML `<style>` element's RAWTEXT body (tokenizer does not expand them).
  ...
  if (REMOTE_CSS_RESOURCE_RE.test(decoded)) return true;
  // Capture complete quoted strings (including leading whitespace the URL
  // parser trims) as well as unquoted tokens / `url(https:evil.com/…)` forms.
  const urlRe =
    /(?:@import\s+(?:url\(\s*)?|url\(\s*)(?:"([^"]*)"|'([^']*)'|([^"')\s]+))/gi;
  let match;
  while ((match = urlRe.exec(decoded)) !== null) {
    const raw = match[1] ?? match[2] ?? match[3];
    if (raw === undefined) continue;
    const candidate = decodeHtml
      ? normalizeForRemoteUrlCheck(raw)
      : raw
          .replace(URL_IGNORED_WHITESPACE_RE, "")
          .replace(URL_EDGE_C0_OR_SPACE_RE, "")
          .replace(/\\/g, "/");
    if (isRemoteResourceUrlDecoded(candidate)) return true;
  }
  return false;
}
...
function scanRemoteCssResource(html, options) {
  for (const { tag, attrText, styleBody, inSvgContext } of iterateOpenTags(
    ...
    if (
      tag === "style" &&
      styleBody !== undefined &&
      cssTextHasRemoteResource(styleBody, { decodeHtmlRefs: false })
    ) {
      return true;
    }
  }
...
function* iterateOpenTags(html, options = {}) {
  ...
  let framesetOk = true;
  /**
   * Deferred SVG `<style>` yield: collect character data as CSS while scanning
   * children in the data state (SVG style is not HTML RAWTEXT).
   * @type {{ openTag: { tag: string, attrText: string, inSvgContext: boolean, styleBody?: string }, parts: string[] } | null}
   */
  let pendingSvgStyle = null;

  const flushPendingSvgStyle = () => {
    if (!pendingSvgStyle) return null;
    const styleStillOpen = openElements.some(
      (el) => el.tag === "style" && el.ns === "svg",
    );
    if (styleStillOpen) return null;
    pendingSvgStyle.openTag.styleBody = pendingSvgStyle.parts.join("");
    const tag = pendingSvgStyle.openTag;
    pendingSvgStyle = null;
    return tag;
  };
  ...
  while (i < text.length) {
    ...
    if (start === -1) {
      if (pendingSvgStyle && i < text.length) {
        pendingSvgStyle.parts.push(text.slice(i));
      }
      break;
    }
    // Character data inside a pending SVG `<style>` is stylesheet text.
    if (pendingSvgStyle && start > i) {
      pendingSvgStyle.parts.push(text.slice(i, start));
    }
...
function scanRemoteResourceTag(html, options) {
  for (const { tag, attrText, inSvgContext } of iterateOpenTags(html, options)) {
    ...
    // Only `<input type=image>` fetches `src` / `srcset`; other input types do not.
