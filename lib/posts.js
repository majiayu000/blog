 ...
 * Browsers decode &colon; / &#x3a; / &#58; etc. prior to fetching; raw-text
 * checks would otherwise miss https&colon;//… remote resources.
 */
...
function decodeHtmlCharacterReferences(value) {
  return String(value).replace(
    ...
    (entity, body) => {
      ...
      return HTML_NAMED_REFS.get(body.toLowerCase()) ?? entity;
    },
  );
}
...
/**
 * Decode entities then strip ASCII whitespace so htt&tab;ps:// and numeric
 * tab/LF/CR/FF variants cannot evade remote-URL classification.
 */
function normalizeForRemoteUrlCheck(value) {
  return decodeHtmlCharacterReferences(value).replace(ASCII_WHITESPACE_RE, "");
}
...
/** Absolute http(s) or protocol-relative // URL (after entity decode + whitespace strip). */
function isRemoteResourceUrl(value) {
  return isRemoteResourceUrlDecoded(normalizeForRemoteUrlCheck(value));
}

const SRCSET_DESCRIPTOR_RE = /^\d+(\.\d+)?x$|^\d+w$/i;

/**
 * Yield URL tokens from an already-decoded srcset / imagesrcset value.
 * data: URLs keep internal commas (media-type / payload separator and payload
 * content); only candidate-boundary commas after an optional descriptor split.
 */
function* srcsetUrlTokens(input) {
  ...
  const skipDescriptors = () => {
    ...
    while (i < input.length && input[i] !== ",") {
      const start = i;
      while (i < input.length && !/\s/.test(input[i]) && input[i] !== ",") i += 1;
  ...
  while (i < input.length) {
    ...
    if (input.slice(i, i + 5).toLowerCase() === "data:") {
      ...
      let end = i;
      while (end < input.length && input[end] !== "," && !/\s/.test(input[end])) end += 1;
      url = input.slice(i, end);
      i = end;
    }
...
function srcsetHasRemoteUrl(value) {
  // Normalize before tokenizing so internal whitespace cannot split "https".
  for (const url of srcsetUrlTokens(normalizeForRemoteUrlCheck(value))) {
    if (isRemoteResourceUrlDecoded(url)) return true;
  }
  return false;
}
...
function parseHtmlAttrs(attrText) {
  ...
  let match;
  while ((match = ATTR_TOKEN_RE.exec(attrText)) !== null) {
...
function hasRemoteResourceTag(html) {
  ...
  for (const match of html.matchAll(OPEN_TAG_RE)) {
    const tag = match[1].toLowerCase();
    if (!REMOTE_RESOURCE_TAGS.has(tag)) continue;
