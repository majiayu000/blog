...
function cssTextHasRemoteResource(cssText, options = {}) {
  ...
  const decoded = stripCssComments(decodeCssEscapes(withHtmlRefs));
  ...
  const checkCandidate = (raw) =>
    isRemoteResourceUrlDecoded(normalizeCssUrlCandidate(raw, decodeHtml));

  while (i < decoded.length) {
    const ch = decoded[i];
    if (quote) {
      if (ch === "\\") {
        i = Math.min(decoded.length, i + 2);
        continue;
      }
      if (ch === quote) quote = null;
      i += 1;
    ...
    if (ch === "@" || isIdentStart(ch) || ch === "-") {
      ...
      if (ident === "url") {
        ...
        if (decoded[k] === "(") {
          ...
          let end = k;
          while (end < decoded.length && decoded[end] !== ")" && !/[\t\n\r\f ]/.test(decoded[end])) {
            end += 1;
          }
          if (end > k && checkCandidate(decoded.slice(k, end))) return true;
          i = end;
...
function* iterateOpenTags(html, options = {}) {
  ...
  while (i < text.length) {
    ...
    // In "in frameset" mode, RAWTEXT openers (except script) are ignored — entering
    // RAWTEXT would skip past a following remote `<frame>` the browser loads.
    ...
    // In-body `html` start tags merge into the root and are not pushed. A synthetic
    // special `html` would block later `</foreignObject>` closures.
    if (elementNs === "html" && tag === "html") {
      yield openTag;
      i = j + 1;
      continue;
    }
...
function scanRemoteResourceTag(html, options) {
  for (const { tag, attrText, inSvgContext } of iterateOpenTags(html, options)) {
    ...
    for (const attr of REMOTE_RESOURCE_ATTRS) {
      ...
      if (attr === "srcset" && tag !== "img" && tag !== "source") continue;
