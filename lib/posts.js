...
const REMOTE_RESOURCE_TAGS = new Set([
  ...
  "object",
  "base",
  // `<input type=image>` loads `src` / `srcset` like an image control.
  "input",
]);
...
/**
 * Legacy HTML `background` content attribute (maps to CSS background-image)
 * on table-model / body elements — still loads in browsers.
 */
const LEGACY_BACKGROUND_ATTR_TAGS = new Set([
  "body",
  "table",
  "tbody",
  "thead",
...
/**
 * SVG presentation attributes whose values are CSS and may contain `url(...)`.
 * Checked like style attributes (after HTML character-reference decode).
 */
const SVG_PRESENTATION_URL_ATTRS = new Set([
  "clip-path",
  "cursor",
  "fill",
  "filter",
...
/**
 * Start tags the HTML "in select" insertion mode actually processes.
 * Everything else is ignored (not pushed) — including `<div>`, which would
 * otherwise block `</select>` via the special-tag end-tag rules.
 */
const SELECT_PROCESSED_START_TAGS = new Set([
  "html",
  "option",
  "optgroup",
  "hr",
 ...
 * `frame` is forbidden like `iframe` (legacy frameset browsing contexts).
 */
const FORBIDDEN_EMBED_TAGS = new Set(["iframe", "frame", "embed", "object", "base"]);
...
/**
 * Strip CSS block comments outside of strings/escapes, replacing each with a
 * single space. `/*` inside a quoted string is ordinary content.
 */
...
/**
 * HTML start tags that imply `</p>` when a `p` is in button scope (in-body
 * insertion mode). Closing the paragraph before pushing the new block keeps
 * foreign-content end tags from looking mismatched.
 */
const HTML_CLOSE_P_START_TAGS = new Set([
  "address",
  "article",
  "aside",
  "blockquote",
...
/** Bogus-DOCTYPE / bogus-comment: first `>` ends it; quotes are ordinary. */
function skipUntilGt(text, from) {
 ...
 * only while still in a valid DOCTYPE state.
 *
 * text/html never recognizes XML internal subsets: any `[` (after the name or
 * after a public/system identifier) enters the bogus-DOCTYPE state and the
 * first `>` ends the declaration. Unexpected tokens after the name (e.g. `FOO`)
 * also enter bogus-DOCTYPE — quotes there are ordinary characters.
 */
function skipBogusCommentOrDeclaration(text, start) {
  // `<!DOCTYPE` (ASCII case-insensitive) — state-aware closer.
  if (
    text[start + 1] === "!" &&
    asciiStartsWith(text, start + 2, "doctype")
  ) {
    let i = start + 9; // past `<!DOCTYPE`
    // HTML requires whitespace (or `>` / EOF) after the DOCTYPE keyword.
    // `<!DOCTYPEhtml…>` immediately enters the bogus-DOCTYPE state where
    // quotes are ordinary and the first `>` ends the declaration.
    if (i < text.length && text[i] !== ">" && !isAsciiWhitespace(text[i])) {
      return skipUntilGt(text, i);
    }
    ...
    /** Read a quoted public/system identifier; false if missing/unclosed. */
    const readQuotedId = () => {
    ...
    /** True when `keyword` at `i` is followed by a DOCTYPE delimiter. */
    const keywordAt = (keyword) => {
      if (!asciiStartsWith(text, i, keyword)) return false;
    ...
    if (keywordAt("public")) {
      i += 6;
      skipWs();
      if (!readQuotedId()) return skipUntilGt(text, i);
      // Optional system identifier requires whitespace after the public id.
      // Adjacent quotes (`PUBLIC "x""`) enter bogus-DOCTYPE in browsers.
      if (i < text.length && isAsciiWhitespace(text[i])) {
        skipWs();
    ...
    } else if (keywordAt("system")) {
      i += 6;
      skipWs();
      if (!readQuotedId()) return skipUntilGt(text, i);
      skipWs();
    } else {
      // Not PUBLIC/SYSTEM — bogus DOCTYPE (quotes are ordinary).
      return skipUntilGt(text, i);
    }

    // After valid identifiers: `>` ends; anything else (including `[`) is bogus.
    if (i < text.length && text[i] === ">") return i + 1;
    return skipUntilGt(text, i);
  }
  return skipUntilGt(text, start + 2);
}
 ...
 * Returns `{ after, bodyEnd }` where `bodyEnd` is the start of the validated
 * end tag (not the first textual `</tag` prefix such as `</stylex>`).
 */
function skipRawTextBody(text, from, tag) {
  ...
  while (pos < text.length) {
    ...
    if (closeAt === -1) return { after: text.length, bodyEnd: text.length };
    const after = endTagCloseIndex(text, closeAt, tag);
    if (after !== -1) return { after, bodyEnd: closeAt };
    pos = closeAt + 1;
  }
  return { after: text.length, bodyEnd: text.length };
}
...
const HTML_NAMED_REFS = new Map([
  ...
  ["newline", "\n"],
  ["excl", "!"],
  // Density / prose punctuation used inside srcset descriptors (`1&period;5x`).
  ["period", "."],
]);
...
/**
 * Decode CSS escapes so `url(https\3a //evil.com)` / `url(https\:\/\/…)` cannot
 * evade the remote CSS gate. Hex escapes consume one optional whitespace;
...
/**
 * Advance past a CSS escape at `i` (must point at `\`) without interpreting the
 * decoded code point as tokenizer syntax. Escaped quotes therefore stay inside
 * tokens instead of opening/closing strings.
 */
function skipCssEscape(text, i) {
  ...
  if (/[0-9a-fA-F]/.test(next)) {
    let k = i + 1;
    let n = 0;
    while (k < text.length && n < 6 && /[0-9a-fA-F]/.test(text[k])) {
      k += 1;
      n += 1;
    }
    if (k < text.length) {
...
/** Normalize a CSS URL token for remote checks. */
function normalizeCssUrlCandidate(raw, decodeHtml) {
  return decodeHtml
    ? normalizeForRemoteUrlCheck(raw)
    : String(raw)
...
/**
 * Scan CSS text outside of strings for remote `url()` / `@import` / `image-set`
 * string candidates. Strings are tracked so `content:"url(https://…)"` is not
 * treated as a resource fetch.
 */
function cssTextHasRemoteResource(cssText, options = {}) {
  // HTML character references are decoded in style *attributes* and SVG
  // `<style>` bodies (data state), but not inside an HTML `<style>` RAWTEXT body.
  const decodeHtml = options.decodeHtmlRefs !== false;
  const withHtmlRefs = decodeHtml
    ? decodeHtmlCharacterReferences(cssText)
    : String(cssText);
  // Fail closed when named refs remain unresolved in CSS-bearing text that
  // browsers would decode (`url&lpar;https://…)` → `url(https://…)`).
  if (decodeHtml && /&[A-Za-z][A-Za-z0-9]*;/.test(withHtmlRefs)) return true;
  // Comments may separate `@import` from its URL (`@import/**/"https://…"`).
  // Do NOT decode CSS escapes before quote tracking: `\22 ` is an escaped
  // code point inside a token, not a string delimiter the browser would honor.
  const decoded = stripCssComments(withHtmlRefs);
  ...
  const peekIdent = (from) => {
    let k = from;
    while (k < decoded.length && isIdent(decoded[k])) k += 1;
    return decoded.slice(from, k);
  };

  const skipWs = (from) => {
    let k = from;
    while (k < decoded.length && /[\t\n\r\f ]/.test(decoded[k])) k += 1;
    return k;
  };

  const readQuoted = (from) => {
    const q = decoded[from];
    if (q !== '"' && q !== "'") return null;
    let k = from + 1;
    let out = "";
    while (k < decoded.length) {
      const ch = decoded[k];
      if (ch === "\\") {
        const after = skipCssEscape(decoded, k);
        out += decoded.slice(k, after);
        k = after;
        continue;
      }
      if (ch === q) return { value: out, after: k + 1 };
      out += ch;
      k += 1;
    }
    return { value: out, after: decoded.length };
  };

  const checkCandidate = (raw) =>
    isRemoteResourceUrlDecoded(
      normalizeCssUrlCandidate(decodeCssEscapes(raw), decodeHtml),
    );

  /** Consume a `url(...)` argument list starting at `(` after the ident. */
  const consumeUrlArgs = (openParenAt) => {
    let k = skipWs(openParenAt + 1);
    const quoted = readQuoted(k);
    if (quoted) {
      return {
        remote: checkCandidate(quoted.value),
        after: quoted.after,
      };
    }
    let end = k;
    while (
      ...
      decoded[end] !== ")" &&
      !/[\t\n\r\f ]/.test(decoded[end])
    ) {
      if (decoded[end] === "\\") {
        end = skipCssEscape(decoded, end);
        continue;
      }
      end += 1;
    }
    return {
      remote: end > k && checkCandidate(decoded.slice(k, end)),
      after: end,
    };
  };
  ...
  while (i < decoded.length) {
    ...
    if (ch === "@" || isIdentStart(ch) || ch === "-") {
      if (ch === "@") {
        const name = peekIdent(i + 1).toLowerCase();
        if (name === "import") {
          let k = skipWs(i + 1 + name.length);
          // CSS function names are ASCII-case-insensitive (`UrL(`).
          if (asciiStartsWith(decoded, k, "url") && decoded[k + 3] === "(") {
            // Fall through to url( handling by advancing i to 'u'
            i = k;
            continue;
          }
          const quoted = readQuoted(k);
          if (quoted && checkCandidate(quoted.value)) return true;
          // Unquoted @import token
          if (!quoted) {
            let end = k;
            while (
              end < decoded.length &&
            ...
            if (end > k && checkCandidate(decoded.slice(k, end))) return true;
            i = Math.max(end, k + 1);
            continue;
          }
          i = quoted.after;
      ...
      const ident = peekIdent(i).toLowerCase();
      if (ident === "url") {
        let k = skipWs(i + ident.length);
        if (decoded[k] === "(") {
          const consumed = consumeUrlArgs(k);
          if (consumed.remote) return true;
          i = consumed.after;
          continue;
        }
      }

      if (ident === "image-set" || ident === "-webkit-image-set") {
        let k = skipWs(i + ident.length);
        if (decoded[k] === "(") {
          k += 1;
          let depth = 1;
          ...
          while (k < decoded.length && depth > 0) {
            ...
            if (c === '"' || c === "'") {
              const quoted = readQuoted(k);
              if (quoted) {
                if (checkCandidate(quoted.value)) return true;
                k = quoted.after;
                continue;
              }
              q2 = c;
            ...
            // Nested `url()` candidates inside image-set(...).
            if (isIdentStart(c) || c === "-") {
              const nestedIdent = peekIdent(k).toLowerCase();
              if (nestedIdent === "url") {
                let u = skipWs(k + nestedIdent.length);
                if (decoded[u] === "(") {
                  const consumed = consumeUrlArgs(u);
                  if (consumed.remote) return true;
                  k = consumed.after;
                  continue;
                }
...
/**
 * Remote CSS via `url()` / `@import`, after HTML character-reference and
 * CSS-escape decoding (covers `url(https&colon;//…)` and `url(https\3a //…)`).
 *
 * Only style attributes and `<style>` element bodies are scanned — never the
 * whole document (script/prose strings must not be interpreted as CSS).
 */
function hasRemoteCssResource(html) {
  for (const noscriptRawtext of [false, true]) {
    if (scanRemoteCssResource(html, { noscriptRawtext })) return true;
  }
  return false;
}

/**
 * Scan style attributes, SVG presentation attrs with `url()`, and `<style>`
 * element bodies (HTML and SVG) for remote CSS. Relies on `iterateOpenTags`
 * open-element tracking so RAWTEXT/`select`/`frameset` rules match the
 * resource-tag scanner.
 */
function scanRemoteCssResource(html, options) {
  for (const { tag, attrText, styleBody, inSvgContext } of iterateOpenTags(
    html,
    options,
  )) {
    const attrs = parseHtmlAttrs(attrText);
    if ("style" in attrs && cssTextHasRemoteResource(attrs.style)) return true;
    // Presentation attributes are CSS-bearing only in the SVG namespace.
    if (inSvgContext) {
      for (const name of SVG_PRESENTATION_URL_ATTRS) {
        if (name in attrs && cssTextHasRemoteResource(attrs[name])) return true;
      }
    }
    if (
      tag === "style" &&
      styleBody !== undefined &&
      cssTextHasRemoteResource(styleBody, {
        // HTML `<style>` is RAWTEXT (no entity decode). SVG `<style>` is data
        // state, so character references are decoded before CSS parsing.
        decodeHtmlRefs: inSvgContext,
      })
    ) {
      return true;
    }
...
function* iterateOpenTags(html, options = {}) {
  ...
  const openElements = [];
  /**
   * HTML `frameset-ok` flag. Non-whitespace character data sets it false so a
   * later `<frameset>` is ignored (not pushed) — matching the browser.
   */
  let framesetOk = true;
  /**
   * Deferred SVG `<style>` yields (stack): nested SVG `<style>` elements each
   * collect their own character data. A single pending record would let an
   * inner close discard the outer stylesheet text.
   * @type {Array<{ openTag: { tag: string, attrText: string, inSvgContext: boolean, styleBody?: string }, parts: string[] }>}
   */
  const pendingSvgStyles = [];

  const flushPendingSvgStyle = () => {
    if (pendingSvgStyles.length === 0) return null;
    // Count open SVG style elements still on the stack.
    let openSvgStyles = 0;
    for (const el of openElements) {
      if (el.tag === "style" && el.ns === "svg") openSvgStyles += 1;
    }
    if (pendingSvgStyles.length <= openSvgStyles) return null;
    const pending = pendingSvgStyles.pop();
    pending.openTag.styleBody = pending.parts.join("");
    return pending.openTag;
  };

  const currentPendingSvgStyle = () =>
    pendingSvgStyles.length > 0
      ? pendingSvgStyles[pendingSvgStyles.length - 1]
      : null;
  ...
  /** True while an HTML `<select>` is on the stack ("in select" insertion mode). */
  const inSelectMode = () => {
    for (let k = openElements.length - 1; k >= 0; k -= 1) {
      const el = openElements[k];
      if (el.ns === "html" && el.tag === "select") return true;
    }
    return false;
  };
  /** True while an HTML `<frameset>` is on the stack ("in frameset" mode). */
  const inFramesetMode = () => {
    for (let k = openElements.length - 1; k >= 0; k -= 1) {
      const el = openElements[k];
      if (el.ns === "html" && el.tag === "frameset") return true;
    }
    return false;
  };

  /**
   * Close an open `<p>` when a block start tag implies `</p>` (in-body rules).
   * Without this, a stale special `p` makes later foreign end tags look
   * mismatched and can hide remote resources behind RCDATA.
   */
  const closeImpliedP = () => {
    for (let k = openElements.length - 1; k >= 0; k -= 1) {
      const el = openElements[k];
      if (el.ns === "html" && el.tag === "p") {
        openElements.length = k;
        return;
      }
      // Button scope markers: stop searching without closing a p outside scope.
      if (
        el.ns === "html" &&
        (el.tag === "button" ||
          el.tag === "html" ||
  ...
  /**
   * Close an open `<li>` before stacking another (HTML list-item rules).
   * Without this, a stale special `li` can make `</foreignObject>` look
   * mismatched and hide remote resources behind RCDATA.
   */
  const closeImpliedLi = () => {
    for (let k = openElements.length - 1; k >= 0; k -= 1) {
      const el = openElements[k];
      if (el.ns === "html" && el.tag === "li") {
        openElements.length = k;
        return;
      }
      // List item scope markers (subset used by the tree builder).
      if (
        el.ns === "html" &&
        (el.tag === "html" ||
          el.tag === "table" ||
  ...
  /**
   * Close an open `<button>` before stacking another (in-body nested-button
   * rule). A stale special `button` makes `</foreignObject>` look mismatched.
   */
  const closeImpliedButton = () => {
    for (let k = openElements.length - 1; k >= 0; k -= 1) {
      const el = openElements[k];
      if (el.ns === "html" && el.tag === "button") {
        openElements.length = k;
        return;
      }
  ...
  /**
   * Close an open `<dd>` / `<dt>` before stacking another (in-body rules).
   * Without this, a stale special `dd`/`dt` can block `</foreignObject>` and
   * hide a later remote `<img>` behind HTML RCDATA in SVG `<title>`.
   */
  const closeImpliedDdOrDt = () => {
    for (let k = openElements.length - 1; k >= 0; k -= 1) {
      const el = openElements[k];
      if (el.ns === "html" && (el.tag === "dd" || el.tag === "dt")) {
        openElements.length = k;
        return;
      }
      // List-item / button-ish scope markers used for dd/dt.
      if (
        el.ns === "html" &&
        (el.tag === "html" ||
          el.tag === "table" ||
  ...
  const isAnnotationXmlHtmlIntegration = (attrText) => {
    ...
    // Surrounding whitespace is significant — do not trim.
    const folded = asciiLowerCase(decodeHtmlCharacterReferences(encoding));
  ...
  const popEndTag = (endTag) => {
    ...
    // Table end tags use table scope: pop through foster-parented special nodes
    // (e.g. `<table><div></table>`) instead of bailing at the first special.
    if (endTag === "table") {
      for (let j = openElements.length - 1; j >= 0; j -= 1) {
        const node = openElements[j];
        if (node.ns === "html" && node.tag === "table") {
        ...
        if (
          ...
          (node.tag === "html" ||
            node.tag === "template" ||
            node.tag === "#html-integration")
        ) {
          // Left table scope without finding a table — ignore the end tag.
          return;
        }
      }
      return;
  ...
  while (i < text.length) {
    ...
    if (start === -1) {
      const pending = currentPendingSvgStyle();
      if (pending && i < text.length) {
        pending.parts.push(text.slice(i));
      }
      break;
    }
    // Character data inside a pending SVG `<style>` is stylesheet text.
    {
      const pending = currentPendingSvgStyle();
      if (pending && start > i) {
        pending.parts.push(text.slice(i, start));
      }
    }
    // Non-whitespace character data clears frameset-ok (in-body rules).
    if (framesetOk) {
      for (let t = i; t < start; t += 1) {
        const ch = text[t];
        if (ch !== "\0" && !isAsciiWhitespace(ch)) {
          framesetOk = false;
          break;
        }
      }
    ...
    if (
      ...
      // CDATA contents are character data — include them in a pending SVG style.
      const cdataEnd = text.indexOf("]]>", start + 9);
      const payloadEnd = cdataEnd === -1 ? text.length : cdataEnd;
      const pending = currentPendingSvgStyle();
      if (pending) {
        pending.parts.push(text.slice(start + 9, payloadEnd));
      }
    ...
    if (next === "/") {
      ...
      const flushedStyle = flushPendingSvgStyle();
      if (flushedStyle) yield flushedStyle;
      continue;
    }
    ...
    // `svg` / `math` only switch namespace under HTML insertion rules (plus the
    // MathML `annotation-xml` → SVG exception). Nested `<math>` inside SVG stays SVG.
    if (tag === "svg") {
      if (parentNs === "html") {
        elementNs = "svg";
      } else if (
        ...
        openElements[openElements.length - 1].tag === "annotation-xml" &&
        openElements[openElements.length - 1].ns === "math"
      ) {
        elementNs = "svg";
      } else {
        elementNs = parentNs;
      ...
      elementNs = parentNs === "html" ? "math" : parentNs;
    } else if (
      parentNs === "html" &&
    ...
    /** @type {{ tag: string, attrText: string, inSvgContext: boolean, styleBody?: string }} */
    const openTag = { tag, attrText, inSvgContext: parentNs === "svg" };

    // Ignored `<frameset>` when frameset-ok is false: do not enter frameset mode.
    const ignoreFrameset =
      tag === "frameset" && elementNs === "html" && !framesetOk;
    if (ignoreFrameset) {
      yield openTag;
      i = j + 1;
      continue;
    ...
    // In "in select" insertion mode, only a small set of start tags is processed;
    // everything else is ignored (not pushed). A pushed special `<div>` would
    // block `</select>` via HTML end-tag rules and leave select mode stuck so a
    // following `<style>`/`<img>` pair is never scanned correctly.
    if (
      inSelectMode() &&
      parentNs === "html" &&
      !SELECT_PROCESSED_START_TAGS.has(tag)
    ) {
      yield openTag;
      i = j + 1;
      continue;
    ...
    // Nested `<select>` while already in select: pop until the open select, ignore.
    if (inSelectMode() && parentNs === "html" && tag === "select") {
      for (let k = openElements.length - 1; k >= 0; k -= 1) {
        if (openElements[k].ns === "html" && openElements[k].tag === "select") {
          openElements.length = k;
          break;
        }
      }
    ...
    // `<input>` / `<keygen>` / `<textarea>` close the select, then process the tag.
    if (
      inSelectMode() &&
      parentNs === "html" &&
    ...
    // In "in frameset" mode, RAWTEXT openers (including script) are ignored —
    // entering RAWTEXT would skip past a following remote `<frame>` the browser loads.
    const ignoredInFrameset =
      inFramesetMode() &&
      parentNs === "html" &&
      (RAW_TEXT_TAGS.has(tag) || (tag === "noscript" && noscriptRawtext));
    if (ignoredInFrameset) {
      yield openTag;
      i = j + 1;
      continue;
    }
    ...
    // In-body `html` / `body` / `head` start tags merge or are ignored — they
    // are not pushed. A synthetic special `body`/`head` would block later
    // `</foreignObject>` closures the same way a synthetic `html` would.
    if (elementNs === "html" && (tag === "html" || tag === "body" || tag === "head")) {
      yield openTag;
      i = j + 1;
      continue;
    }

    // Outside templates, a second HTML `form` start tag is ignored while a form
    // element pointer already exists. Pushing nested forms leaves a stale
    // special `form` that blocks `</foreignObject>`.
    if (elementNs === "html" && tag === "form") {
      let inTemplate = false;
      let hasForm = false;
      for (const el of openElements) {
        if (el.ns === "html" && el.tag === "template") inTemplate = true;
        if (el.ns === "html" && el.tag === "form") hasForm = true;
      }
      if (hasForm && !inTemplate) {
        yield openTag;
        i = j + 1;
        continue;
      }
    ...
    // Start tags that clear frameset-ok (subset of in-body rules). Without this,
    // a leading `<hr>` leaves frameset-ok true so a later `<frameset>` hides
    // a RAWTEXT `<style>` and a remote `<img>` after an unmatched quote.
    if (
      framesetOk &&
      elementNs === "html" &&
      (tag === "hr" ||
        ...
        tag === "listing" ||
        (tag === "input" &&
          asciiLowerCase(decodeHtmlCharacterReferences(parseHtmlAttrs(attrText).type ?? "text")) !==
            "hidden"))
    ) {
      framesetOk = false;
    }

    // Implied `</p>` before block elements (including void `hr`) so a stale
    // special `p` cannot make `</foreignObject>` look mismatched.
    if (elementNs === "html" && HTML_CLOSE_P_START_TAGS.has(tag)) {
      closeImpliedP();
    }
    // Implied `</li>` before a nested list item (same list-item scope rules).
    if (elementNs === "html" && tag === "li") {
      closeImpliedLi();
    }
    // Implied `</button>` before a nested button.
    if (elementNs === "html" && tag === "button") {
      closeImpliedButton();
    }
    // Implied `</dd>` / `</dt>` before another dd/dt.
    if (elementNs === "html" && (tag === "dd" || tag === "dt")) {
      closeImpliedDdOrDt();
    }
    ...
    // is an integration point, not HTML RCDATA). SVG `<style>` is NOT HTML
    // RAWTEXT — scan its children in the data/foreign-content states (so an
    // `<img>` breakout is visible) while collecting text nodes as CSS.
    const isHtmlRawTextElement =
      (RAW_TEXT_TAGS.has(tag) || (tag === "noscript" && noscriptRawtext)) &&
      parentNs === "html";
    const isSvgStyleElement = tag === "style" && parentNs === "svg";
    ...
    if (isHtmlRawTextElement && !honorSelfClosing) {
      ...
      if (tag === "plaintext") {
        ...
        const bodyStart = i;
        if (tag === "script") {
          i = skipScriptBody(text, i);
        } else {
          const skipped = skipRawTextBody(text, i, tag);
          i = skipped.after;
          if (tag === "style") {
            openTag.styleBody = text.slice(bodyStart, skipped.bodyEnd);
          }
        }
    ...
    if (isSvgStyleElement && !honorSelfClosing) {
      // Defer yielding until the style element closes so styleBody includes
      // collected character data; child breakout tags are still yielded first.
      pendingSvgStyles.push({ openTag, parts: [] });
      continue;
    }
  ...
  // Unclosed SVG `<style>` at EOF — still expose collected CSS text.
  while (pendingSvgStyles.length > 0) {
    const pending = pendingSvgStyles.pop();
    pending.openTag.styleBody = pending.parts.join("");
    yield pending.openTag;
  }
}
 ...
 * floating-point number (including a leading minus and scientific notation
 * like `1e0x` / `-0x`) then `x`; values less than zero are rejected when
 * validating the token. A trailing decimal point without a following digit
 * (`1.x`, `1.e2x`) is invalid. Width/height are positive integers then
 * `w` / `h` (`0w`/`0h` are invalid and discarded; `h` is reserved for
 * future compatibility but still delimits candidates).
 */
...
function* srcsetUrlTokens(input) {
  ...
  const skipDescriptors = () => {
    ...
    while (i < input.length && input[i] !== ",") {
      ...
      // Density must not be less than zero (`-0x` is valid; `-1x` is not).
      if (token.endsWith("x")) {
        const density = Number.parseFloat(token.slice(0, -1));
...
function srcsetHasRemoteUrl(value) {
  ...
  // Fail closed when named refs remain unresolved anywhere in the value —
  // including descriptors (`1&period;5x`). Leaving them undecoded makes the
  // descriptor look invalid and would skip a remote URL the browser fetches.
  if (/&[A-Za-z][A-Za-z0-9]*;/.test(decoded)) return true;
...
function scanRemoteResourceTag(html, options) {
  for (const { tag, attrText, inSvgContext } of iterateOpenTags(html, options)) {
    const attrs = parseHtmlAttrs(attrText);
    // Legacy `background` on body/table cells loads like CSS background-image.
    if (
      LEGACY_BACKGROUND_ATTR_TAGS.has(tag) &&
      "background" in attrs &&
      attrHasRemoteResource("background", attrs.background)
    ) {
      return true;
    }
    ...
    // Only `<input type=image>` fetches `src`; other input types do not.
    // Enumerated `type` keywords are ASCII-case-insensitive but not whitespace-
    // trimmed — surrounding spaces make the value invalid (text state).
    // `srcset` is img/source only; `imagesrcset` is image preloads (`link`).
    if (tag === "input") {
      const type = asciiLowerCase(
        decodeHtmlCharacterReferences(attrs.type ?? "text"),
      );
      if (type !== "image") continue;
      if ("src" in attrs && attrHasRemoteResource("src", attrs.src)) return true;
      continue;
    }
    for (const attr of REMOTE_RESOURCE_ATTRS) {
      if (!(attr in attrs)) continue;
      // HTML `<image>` is rewritten to `<img>` and can fetch `srcset`; SVG
      // `<image>` uses href/xlink:href instead and is handled via that path.
      if (
        attr === "srcset" &&
        tag !== "img" &&
        tag !== "source" &&
        !(tag === "image" && !inSvgContext)
      ) {
        continue;
      }
      if (attr === "imagesrcset" && tag !== "link") continue;
      if (attrHasRemoteResource(attr, attrs[attr])) return true;
    }
  }
  return false;
}
...
function assertSelfContained(html, where) {
  if (hasRemoteResourceTag(html) || hasRemoteCssResource(html)) {
