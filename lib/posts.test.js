...
test("外部资源依赖报错，但普通来源链接合法", () => {
  ...
  const dependencies = [
    ...
    // CSS: HTML entity / CSS-escape bypasses of the raw url() regex
    `<p style="background:url(https&colon;//evil.com/x.png)">x</p>`,
    `<style>div{background:url(https\\3a //evil.com/x.png)}</style>`,
    `<style>@import url("https\\3a //evil.com/x.css")</style>`,
    // `<input type=image>` loads src like an image (srcset is ignored on input)
    `<input type=image src=https://evil.com/x.png>`,
    // Legacy `<frame>` embeds like iframe
    `<frame src=https://evil.com/x.html>`,
    `<frameset><frame src=https://evil.com/x.html></frameset>`,
    // In-select mode ignores `<style>` RAWTEXT; following img still loads
    `<select><style></select><img src=https://evil.com/x.png></style>`,
    // Bare `[` after DOCTYPE name is bogus DOCTYPE (ends at first `>`)
    `<!DOCTYPE html [><img src=https://evil.com/x.png>]>`,
    // `[` after a completed public/system id is also bogus in text/html
    `<!DOCTYPE html SYSTEM "x" [><img src=https://evil.com/x.png>]>`,
    `<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01//EN" "x" [><img src=https://evil.com/x.png>]>`,
    // Implied `</p>` before block elements inside SVG foreignObject
    `<svg><foreignObject><p><div></div></foreignObject><title><img src=https://evil.com/a.png></title></svg>`,
    // In-frameset mode ignores `<style>` RAWTEXT; following frame still loads
    `<frameset><style><frame src=https://evil.com/x.html></style></frameset>`,
    // Named refs in srcset descriptors must decode (`1&period;5x` → `1.5x`)
    `<img srcset="https://evil.com/x.png 1&period;5x">`,
    // annotation-xml encoding whitespace is significant (not an HTML integration point)
    `<math><annotation-xml encoding=" text/html"><title><img src=https://evil.com/x.png></title></annotation-xml></math>`,
    // Bogus DOCTYPE: quotes are ordinary after an unexpected token (`FOO`)
    `<!DOCTYPE html FOO "><img src=https://evil.com/x.png">`,
    // SVG presentation attributes accept CSS url() values
    `<svg><rect filter="url(https://evil.com/filter.svg#f)"/></svg>`,
    `<svg><rect fill="url(https://evil.com/paint.svg#p)"/></svg>`,
    // Nested <li> implies closure of the earlier list item (foreignObject breakout)
    `<svg><foreignObject><li><li></li></foreignObject><title><img src=https://evil.com/a.png></title></svg>`,
    // SVG-namespace <style> still creates an active stylesheet
    `<svg><style>@import "https://evil.com/x.css";</style></svg>`,
    `<svg><style>.x{background:url(https://evil.com/x.png)}</style></svg>`,
    // styleBody must use the validated </style> end tag, not </stylex>
    `<style>/*</stylex>*/ @import "https://evil.com/x.css";</style>`,
    // Negative-zero density descriptor is valid (`-0x`)
    `<img srcset="https://evil.com/x.png -0x">`,
    // frameset-ok cleared by leading text — ignored frameset must not hide <style>
    `x<frameset><style><fake a="</style><img src=https://evil.com/x.png>`,
    // frameset-ok cleared by start tags such as hr
    `<hr><frameset><style><fake a="</style><img src=https://evil.com/x.png>`,
    // Table end-tag pops through foster-parented special nodes
    `<svg><foreignObject><table><div></table></foreignObject><title><img src=https://evil.com/a.png></title></svg>`,
    // Incomplete named refs in CSS attributes must fail closed
    `<p style="background:url&lpar;https://evil.com/x.png)">x</p>`,
    // image-set / -webkit-image-set string candidates (no nested url())
    `<style>.x{background-image:image-set("https://evil.com/x.png" 1x)}</style>`,
    `<style>.x{background-image:-webkit-image-set("https://evil.com/x.png" 1x)}</style>`,
    // CSS comment delimiters inside strings must not strip real url() rules
    `<style>.x{--a:"/*";background:url(https://evil.com/x.png);--b:"*/"}</style>`,
    // DOCTYPE: adjacent quote after public id is bogus (not a system id)
    `<!DOCTYPE html PUBLIC "x""><img src=https://evil.com/x.png>`,
    // Nested html start tag inside foreignObject must not block closures
    `<svg><foreignObject><html></foreignObject><title><img src=https://evil.com/x.png></title></svg>`,
    // Nested SVG <style> must not discard the outer stylesheet text
    `<svg><style>@import "https://evil.com/x.css";<style></style></style></svg>`,
    // Nested math under svg stays in SVG namespace
    `<svg><math><mtext><title><img src=https://evil.com/x.png></title></mtext></math></svg>`,
    // SVG style bodies decode HTML character references
    `<svg><style>.x{fill:url(https&colon;//evil.com/x.svg#p)}</style><rect class=x/></svg>`,
    // SVG CDATA inside <style> is stylesheet character data
    `<svg><style><![CDATA[.x{fill:url(https://evil.com/x.svg#p)}]]></style><rect class=x/></svg>`,
    // Nested button implies closure of the earlier button
    `<svg><foreignObject><button><button></button></foreignObject><title><img src=https://evil.com/x.png></title></svg>`,
    // Legacy background attribute on table/body cells
    `<body background="https://evil.com/bg.png">`,
    `<table background="https://evil.com/bg.png"><tr><td>x</td></tr></table>`,
    `<td background="https://evil.com/bg.png">x</td>`,
    // In-select: unsupported start tags (e.g. div) are ignored, not pushed
    `<select><div></select><style><fake a="</style><img src=https://evil.com/x.png>`,
    // CSS comments may separate @import from its URL
    `<style>@import/**/"https://evil.com/x.css";</style>`,
    `<style>@import/**/url(https://evil.com/x.css);</style>`,
    // SVG cursor presentation attribute accepts url(...)
    `<svg><rect cursor="url(https://evil.com/x.cur),auto"/></svg>`,
    // SVG <style> is not HTML RAWTEXT — img breakout must be visible
    `<svg><style><img src=https://evil.com/x.png></style></svg>`,
    // Quoted CSS url() trims leading whitespace before resolving
    `<style>.x{background:url(" https://evil.com/x.png")}</style>`,
    `<style>@import url(" https://evil.com/x.css");</style>`,
    // Escaped quote code point must not open a CSS string (Codex P1)
    `<style>.x{--a:\\22 ;background:url(https://evil.com/x.png)}</style><div class=x></div>`,
    // DOCTYPE without whitespace after keyword is bogus (quotes ordinary)
    `<!DOCTYPEhtml SYSTEM "><img src=https://evil.com/x.png>">`,
    // In frameset mode, script start tags are ignored like other RAWTEXT
    `<frameset><script><frame src=https://evil.com/x></script></frameset>`,
    // Nested dd/dt imply closure so foreignObject can close
    `<svg><foreignObject><dd><dt></dt></foreignObject><title><img src=https://evil.com/x></title></svg>`,
    // Nested form start tags are ignored outside templates
    `<svg><foreignObject><form><form></form></foreignObject><title><img src=https://evil.com/x></title></svg>`,
    // @import url() function name is ASCII-case-insensitive
    `<style>@import UrL(https://evil.com/x.css);</style>`,
    // HTML <image> is an img alias and can fetch srcset
    `<image srcset="https://evil.com/x.png 1x">`,
    // Nested url() inside image-set / -webkit-image-set
    `<style>.x{background:image-set(url(https://evil.com/x.png) 1x)}</style><div class=x></div>`,
    `<style>.x{background:-webkit-image-set(url(https://evil.com/x.png) 1x)}</style><div class=x></div>`,
    // In-body body/head start tags are not pushed (foreignObject must close)
    `<svg><foreignObject><body></foreignObject><title><img src=https://evil.com/x></title></svg>`,
    `<svg><foreignObject><head></foreignObject><title><img src=https://evil.com/x></title></svg>`,
  ];
  ...
  // url()-like text inside CSS strings is not a resource fetch
  expect(
    parsePost(page(`${metadata}<style>.x{content:"url(https://evil.com/example.png)"}</style>`), "s")
      .title,
  ).toBe("T");
  // input type=image ignores srcset; local src alone is self-contained
  expect(
    parsePost(
      page(`${metadata}<input type=image src=local.png srcset="https://evil.com/x.png 1x">`),
      "s",
    ).title,
  ).toBe("T");
  // HTML presentation-looking attrs do not fetch (SVG-only)
  expect(
    parsePost(page(`${metadata}<div fill="url(https://evil.com/x)"></div>`), "s").title,
  ).toBe("T");
  expect(
    parsePost(page(`${metadata}<div cursor="url(https://evil.com/x.cur),auto"></div>`), "s").title,
  ).toBe("T");
  ...
  // Trailing decimal in density descriptor is invalid (`1.x`); no remote fetch
  expect(
    parsePost(page(`${metadata}<img srcset="https://evil.com/x.png 1.x" alt="">`), "s").title,
  ).toBe("T");
  expect(
    parsePost(page(`${metadata}<img srcset="https://evil.com/x.png 1.e2x" alt="">`), "s").title,
  ).toBe("T");
  // Script strings are not CSS — CSS-escape sequences inside JS must not reject
  expect(
    parsePost(
      page(`${metadata}<script>const sample = "url(https\\3a //evil.com/x.png)";</script>`),
      "s",
    ).title,
  ).toBe("T");
  // input type is not whitespace-trimmed; surrounding spaces → text state (no fetch)
  expect(
    parsePost(page(`${metadata}<input type=" image " src="https://evil.com/x.png">`), "s").title,
  ).toBe("T");
  // HTML character references are not decoded inside <style> RAWTEXT
  expect(
    parsePost(
      page(`${metadata}<style>.x{background:url(https&colon;//evil.com/x.png)}</style>`),
      "s",
    ).title,
  ).toBe("T");
  // Negative density less than zero is invalid and discards the candidate
  expect(
    parsePost(page(`${metadata}<img srcset="https://evil.com/x.png -1x" alt="">`), "s").title,
  ).toBe("T");
});
