...
function rankRelatedPosts(posts, current) {
...
function pickRelatedPosts(ranked, excludedSlugs, limit = 3) {
...
export function getPostJourney(posts, current, relatedLimit = 3) {
  ...
  return {
    newer,
    older,
    related: pickRelatedPosts(rankRelatedPosts(posts, current), journeySlugs, relatedLimit),
  };
}
...
try {
  ...
  if (savedTheme === "light" || savedTheme === "dark") {
    document.documentElement.dataset.ssShellTheme = savedTheme;
  }
} catch (error) {
...
function renderRelatedPosts(related, current) {
  const topic = primaryTopicForPost(current);
  const heading = `<div class="ss-afterword__section-heading">
      <span>// KEEP READING</span>
      <h3 id="ss-related-heading">继续阅读</h3>
    </div>`;

  if (!related.length) {
    return `<section class="ss-afterword__related ss-afterword__related--empty" aria-labelledby="ss-related-heading">
    ${heading}
    <p>本专题暂无更多文章。<a href="/topics/${escapeHtml(topic.slug)}/">查看「${escapeHtml(topic.title)}」</a></p>
  </section>`;
  }

  const items = related
    .map((post, index) => {
      const relatedTopic = primaryTopicForPost(post).title;
      return `<li>
        <a href="${escapeHtml(post.url)}" data-ss-event="related_click" data-ss-slug="${escapeHtml(post.slug)}">
          ...
          <span class="ss-afterword__related-copy">
            <small>${escapeHtml(relatedTopic)}</small>
            <strong>${escapeHtml(post.title)}</strong>
          </span>
...
<aside class="ss-afterword" id="afterword" aria-label="文章后记与评论" data-pagefind-ignore>
  ...
  ${renderRelatedPosts(journey.related, post)}
