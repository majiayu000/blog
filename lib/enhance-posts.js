...
function renderComments(post, comments) {
  ...
  return `<section class="ss-afterword__comments" aria-labelledby="ss-comments-heading">
    ...
    <script class="giscus-script" src="https://giscus.app/client.js"
      data-repo="${escapeHtml(comments.repo)}"
      data-repo-id="${escapeHtml(comments.repoId)}"
      data-category="${escapeHtml(comments.category)}"
      data-category-id="${escapeHtml(comments.categoryId)}"
...
function renderAfterword(post, journey, site) {
...
export function enhancePostHtml(html, post, posts, site, where = post.slug) {
  ...
  return withHead.replace(
    /<\/body\s*>/i,
    `${renderAfterword(post, journey, site)}\n</body>`,
  );
}
